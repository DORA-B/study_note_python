	...
	while changing1:
		...
		for x in range(1, rows - 1):
			for y in range(1, columns - 1):
				...
				intcol = (y+1) if (y < columns - 2 ) else y; # 这里错误，当y恰好是最大时(cloumns-2)，进入下方的函数zerotoone，已经在边界处，里边y1++，错误
				condition4 = (P4 * P6 * P6 == 0) or (zeroToOne(Image_Thinned,x-1,intcol)!=1)#p4
...
if __name__ == "__main__":
	...
	for file in glob.glob(test_path + "/*.png"):
		...
		skeleton = skeleton.astype(np.uint8)* 255 # 注意数组中的数据范围应当都在0-255
