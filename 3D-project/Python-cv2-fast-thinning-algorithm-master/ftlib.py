...
def fastThin(img_ori):
  ...
  if(show_images):
    ...
    cv2.imwrite(dirname+'pre_morphology'+str(time.time())+'.png',img)
  img = morphology(img)
  cleanCorners(img)
  if(show_images):
    cv2.imshow('morphology',img)
    cv2.imwrite(dirname + 'morphology' + str(time.time())+'.png', img)
  eraseTwoByTwos(img)
  if(show_images):
    cv2.imshow('morphology + eraseTwoByTwos',img)
    cv2.imwrite(dirname + 'morphology_+_eraseTwoByTwos' +str(time.time())+'.png', img)
  eraseLadders(img)
  if(show_images):
    cv2.imshow('morphology + eraseTwoByTwos + eraseLadders',img)
    cv2.imwrite(dirname + 'morphology_+_eraseTwoByTwos_+_eraseLadders'+str(time.time())+'.png', img)
  return img
...
# used in Zhang Suen algorithms to eliminate undesired corners 消除不需要的角落
